...
// @scriptsource	http://google.monkeyr.com/script/1.6.3/googlemonkeyr.user.js
/* StartHistory
...
//		document.addEventListener("DOMAttrModified", this.DOMAttrModifiedWatcher.bind(this), false);
//		document.addEventListener("DOMAttrModified", function(e){monkeyr.log(e.target)}, false);
...
//			this.addStyle("@namespace url(http://www.w3.org/1999/xhtml); #rcnt{margin-top:1em} #sfcnt,#sftr,#searchform{display:none!important;}#cnt{padding:0}#cnt .mw:first-child{position:absolute;top:4.5em;right:0}#rshdr .sfcc{position:absolute;top:2em;right:0}");
		...
		{
			monkeyr.log('hereksaksksks')
			this.addStyle("@namespace url(http://www.w3.org/1999/xhtml); #tads, #topstuff table.ts, #bottomads{display:none;}");
		}

		if(UIL.Config.getNoSitePreview())
		{
		...
		if(UIL.Config.getRemRightPanel())
		{
...
//		var prt;
////		if(window.google)
////			monkeyr.log('window.google')
////		monkeyr.log('watchTimer')
//
//		return;
//		try{
//			//monkeyr.log('google.timers.load.t.prt')
//			//monkeyr.log(window.google.timers.load.t.prt)
//			//prt = window.google.timers.load.t.prt;
//		}
//		catch(e){
//			monkeyr.log('watchTimer catch');
//			return;
//		}
//		if(typeof(prt)=='undefined'){
//			monkeyr.log('watchTimer prt == undefined');
//			return;
//		}
//		else if(this.oo == prt){
//			//monkeyr.log('watchTimer oo == prt');
//			return;
//		}
//		else if(typeof(this.oo)=='undefined'){
//			this.oo = +prt;
...
//			if(UIL.Config.getHideSearch())
//			{
//				that.resizeWatcher(true);
//			}
//			that.buildResultTable();
//			if(UIL.Config.getAutoLoad())
//			{
//				if(that.initialiseAutoLoad() && that.insertLoadingImage())
//				{
//					//monkeyr.log('watchForScroll');
//					that.watchForScroll.bind(that)();
//				}
//			}
//
//			that.processResults(document.getElementById('center_col'))
...
//	},
	...
	{
		// monkeyr.log(e.target.id, e.attrName, e.newValue)
...
//				var ires = document.getElementById('ires');
//				if(ires){
//					monkeyr.log('heeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee')
//					monkeyr.log(e.target.id, e.attrName, e.newValue)
//					if(UIL.Config.getHideSearch())
//					{
//						that.resizeWatcher(true);
...
//					if(UIL.Config.getAutoLoad())
//					{
//						that.initialiseAutoLoad();
//						if(that.insertLoadingImage())
//						{
//							//monkeyr.log('watchForScroll');
//							that.watchForScroll.bind(that)();
//						}
...
//		}
	...
	DOMNodeInsertedWatcher : function(e)
	{
		...
		else if((!document.getElementById('GoogleTabledResults') && typeof(e.target.hasAttribute)=='function' && e.target.hasAttribute('class') && e.target.getAttribute('class') == 'vspib'))
		{
...
//	{
////		monkeyr.log(e.target)
//		if((!document.getElementById('GoogleTabledResults') && e.target.hasAttribute('class') && e.target.getAttribute('class') == 'vsc'))
//		{
//			//monkeyr.log(e.target)
//			clearTimeout(this.timcou)
//			that = this;
...
//					that.processResults(document.getElementById('center_col'))
//					var ires = document.getElementById('ires');
//					if(ires) ires.parentNode.removeChild(ires);
//
//				//},0)
//			},0);
//		}
//	},
	...
	rightPanelAdjuster : function(el)
	{
		if(el && (center_col = document.getElementById('center_col')) && (foot = document.getElementById('foot'))){
			monkeyr.log('rhs stuff',  this.remSponsor, this.remRightPanel);
			if((mbEnd = document.getElementById('mbEnd')) && this.remSponsor){
				monkeyr.log('rem mbEnd');
				mbEnd.parentNode.removeChild(mbEnd);
			}
			if((rhs_block = document.getElementById('rhs_block')) && rhs_block.childNodes.length>2 && !this.remRightPanel){
				monkeyr.log('el.offsetWidth:'+el.offsetWidth)
...
UIL.Config =
	...
	getNumCol : function()
	{
		return this._getBooleanConfig("numCol", 2);
	},
	...
	{
		return this._getBooleanConfig("numResults", true);
	},
	...
	{
		return this._getBooleanConfig("autoLoad", true);
	},
	...
	{
		return this._getBooleanConfig("imagePreview", true);
	},
